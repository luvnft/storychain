...
import { UserNovelData } from "common/types"
...
export const getNovelAttestation = async (data: UserNovelData) => {
  ...
  const schemaUid = `0x1016946867805142e4887930d40cc6816778109132ac69016694c116f6bfdcc0`
  const schemaEncoder = new SchemaEncoder(
    "string previousContent, string inputSentence, string outputContent"
  )
  const encodedData = schemaEncoder.encodeData([
    { name: "previousContent", type: "string", value: data.previousContent },
    { name: "inputSentence", type: "string", value: data.inputSentence },
    { name: "outputContent", type: "string", value: data.outputContent },
  ])
